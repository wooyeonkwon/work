...
int main(int argc, char** argv) {
    ...
    // 먼저 파일을 열어 전체 엔트리 수를 확인 (트리는 "AnalysisMC/Analysis"에 존재한다고 가정)
    TFile* f = TFile::Open(rootFileName.c_str(), "READ");
    if (!f || f->IsZombie()) {
        std::cerr << "Cannot open file " << rootFileName << std::endl;
        return 1;
    }
    TDirectory* dir = (TDirectory*)f->Get("AnalysisMC");
    if (!dir) {
    ...
    #pragma omp parallel num_threads(nThreads)
    {
        ...
        if (!threadFile || threadFile->IsZombie()) {
            ...
            TDirectory* threadDir = (TDirectory*)threadFile->Get("AnalysisMC");
            if (!threadDir) {
                #pragma omp critical
                std::cerr << "Thread " << threadID << " cannot find directory 'AnalysisMC'" << std::endl;
            } else {
                ...
                if (!threadTree) {
                    ...
                    // muon MC Matching 정보
                    TTreeReaderValue<std::vector<bool>> muonMCMatched(reader, "muonMCMatched");
                    TTreeReaderValue<bool> muonMCMatchPass(reader, "muonMCMatchPass");
                    ...
                    while (reader.Next()) {
                        ...
                        // MC인 경우, muonMCMatchPass 값이 false이면 이벤트 건너뜀
                        if (!(*muonMCMatchPass)) {
                            ++entry;
                            continue;
                        }
